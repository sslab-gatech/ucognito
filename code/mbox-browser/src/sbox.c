...
struct entry_list {
    struct entry list[ENTRY_MAX];
    int length;
};
struct entry_list clean_dir_list;
struct entry_list write_dir_list;
struct entry_list copy_dir_list;
#undef  ENTRY_MAX
...
static void __sbox_init_dir_lists(void)
{
    clean_dir_list.length = 0;
    write_dir_list.length = 0;
    copy_dir_list.length = 0;
}

void sbox_init(void)
{
    __sbox_init_dir_lists();
    sbox_load_meta();
}
...
static inline
int check_dir_is_proc_dev_sys(struct tcb *tcp, int arg, char *hpn) {

    if (strncmp(hpn, "/dev/", 5) == 0 ||
            strncmp(hpn, "/proc/", 6) == 0 ||
            strncmp(hpn, "/sys/", 5) == 0 ||
            strncmp(hpn, "/etc/", 5) == 0 ||
            strncmp(hpn, "/opt/", 5) == 0)
    {
        // Use host path to ignore /dev and /proc
        sbox_hijack_str(tcp, arg, hpn);
        return 1;
    }
    return 0;
...
void sbox_open_enter(struct tcb *tcp, int fd, int arg, int oflag)
{
    ...
    // Get host path name (hpn) from fd.
    cwd_in_sboxfs = get_hpn_from_fd_and_arg(tcp, fd, arg, hpn, PATH_MAX);

    // Get sandbox path name (spn) from hpn.
    get_spn_from_hpn(hpn, spn, PATH_MAX);
    ...
    // if hpn starts with /dev, /proc, /sys, then return
    if(check_dir_is_proc_dev_sys(tcp, arg, hpn)) {
        return;
    }
    ...
    // File copy has already been copied and hooked at path_exists(spn).
    /*
        At this point, a file is either
            1. in /dev, /proc, /sys, /etc, /opt
            2. set as direct (not for us)
            3. deleted by MBox
            4. exists on sandbox path
            5. Copied by Profile [copy] assertion
    */

    // If mentioned as copy subdir
    {
        int copy_length = copy_dir_list.length;
        int i;
        int hpn_length = strlen(hpn);
        for(i=0; i<copy_length; ++i) {
            if(hpn_length < copy_dir_list.list[i].length)
                continue;
            // If it is matched with copy subdir
            if(strbeg(hpn, copy_dir_list.list[i].path)) {
                // Copy file, and change str.
                byte md5[MD5_DIGEST_LENGTH];
                sbox_sync_parent_dirs(hpn, spn);
                if (copyfile(hpn, spn, md5)) {
                    add_md5_to_map(&os_md5map, hpn, md5);
    ...
    {
        int clean_length = clean_dir_list.length;
        int i;
        int hpn_length = strlen(hpn);
        for(i=0; i<clean_length; ++i) {
            if(hpn_length < clean_dir_list.list[i].length)
                continue;
            // If it is inside of clean directory
            if(strbeg(hpn, clean_dir_list.list[i].path)) {
                sbox_hijack_str(tcp, arg, spn);
                return;
            }
        }
...
int sbox_creat(struct tcb *tcp)
{
    ...
    if (entering(tcp)) {
        ...
        if(check_dir_is_proc_dev_sys(tcp, 0, hpn)) {
            //printf("Creat on proc: %s\n", hpn);
            return 0;
        }
...
int sbox_unlink_general(struct tcb *tcp, int fd, int arg, int flag)
{
    ...
    if (entering(tcp)) {
        ...
        if(check_dir_is_proc_dev_sys(tcp, arg, hpn)) {
            // Do not rewrite path if it is /dev/sys/proc/etc/opt
            return 0;
        }
...
#define is_absolute(path) (path != NULL && path[0] == '/')
...
int sbox_readlink(struct tcb *tcp)
{
    ...
    if (entering(tcp)) {
        umovestr(tcp, tcp->u_arg[0], PATH_MAX, hpn);
        //printf("Readlink is called for [%s]\n", hpn);

        if(check_dir_is_proc_dev_sys(tcp, 0, hpn)) {
        /*
        if (strncmp(hpn, "/dev/", 5) == 0 || strncmp(hpn, "/proc/", 6) == 0) {
        */
            // Do nothing for system related stuffs
            return 0;
        } else {
            if (is_absolute(hpn)) {
                get_spn_from_hpn(hpn, spn, PATH_MAX);
...
int sbox_symlink(struct tcb *tcp)
{
    ...
    /*
       ...
       A new scheme by Yeongjin
       1. If arg1 is an absolute path, change it to spn.
       2. If arg1 is a relative path, do not do anything (does not handle /../)
       3. If arg2 is an absolute path, change it to spn.
       4. If arg2 is a relative path, do not do anything (does not handle /../)
    ...
    */
    ...
    if (entering(tcp)) {
        char first_hpn[PATH_MAX];
        char first_spn[PATH_MAX];
        char second_hpn[PATH_MAX];
        char second_spn[PATH_MAX];

        umovestr(tcp, tcp->u_arg[0], PATH_MAX, first_hpn);
        umovestr(tcp, tcp->u_arg[1], PATH_MAX, second_hpn);
        if (is_absolute(first_hpn)) {
            get_spn_from_hpn(first_hpn, first_spn, PATH_MAX);
        } else {
            strcpy(first_spn, first_hpn);
        }

        if (is_absolute(second_hpn)) {
            get_spn_from_hpn(second_hpn, second_spn, PATH_MAX);
        } else {
            strcpy(second_spn, second_hpn);
        }
        //printf("Create symlink, link [%s], to [%s]\n", first_spn, second_spn);
        sbox_sync_parent_dirs(second_hpn, second_spn);
        sbox_hijack_str(tcp, 0, first_spn);
        sbox_hijack_str(tcp, 1, second_spn);
    }
/*
    if (entering(tcp)) {
...
int sbox_bind(struct tcb *tcp)
{
    if (entering(tcp)) {
        char string[0x200];
        //perror("Entering to bind\n");
        int size = tcp->u_arg[2];

        if (size == sizeof(struct sockaddr_un)) {
            // This is bind for UNIX socket
            struct sockaddr_un *sa = (struct sockaddr_un*) safe_malloc(size);
            if (umoven(tcp, tcp->u_arg[1], size, (char *)sa) < 0) {
            ...
            switch(sa->sun_family) {
                case AF_UNIX:
                {
                    char hpn[PATH_MAX];
                    char spn[PATH_MAX];
                    int size_of_path = sizeof(struct sockaddr_un) - 2;
                    memcpy(hpn, sa->sun_path, size_of_path);
                    get_spn_from_hpn(hpn, spn, PATH_MAX);
                    sbox_sync_parent_dirs(hpn, spn);
                    memcpy(sa->sun_path, spn, size_of_path);

                    sbox_hijack_mem(tcp, 1, sa, sizeof(struct sockaddr_un));
                    break;
                }
                default:
                    break;
        ...
        //sprintf(string, "Sockaddr size %d\n", size);
        //perror(string);
        /*
        struct sockaddr *sa \
...
int sbox_socket(struct tcb *tcp)
{
    if (entering(tcp)) {
        ...
        if (pf == PF_FILE || pf == AF_UNIX) {
            //perror("Unix Socket is added\n");
        }
    }
    return 0;
}
...
static int _sbox_chk_write_policy(char *spn, char *hpn)
{
    //printf("SPN [%s]\n", spn);
    //printf("HPN [%s]\n", hpn);
    // match hpn here.
    int write_length = write_dir_list.length;
    int i;
    int hpn_length = strlen(hpn);
    for(i=0; i<write_length; ++i) {
        // check if current entry is subdir or not.
        struct entry ent = write_dir_list.list[i];
        if(__sbox_check_subdir_path(ent.path)) {
            // if subdir, do prefix match
            if(hpn_length < ent.length) continue;
            if(strbeg(hpn, ent.path)) {
                // write back file.
                printf("Write back [%s]\n", hpn);
                copyfile(spn, hpn, NULL);
            }
            ...
            // if file, do exact match
            if(strcmp(hpn, ent.path) == 0) {
                // write back file here.
                printf("Write back [%s]\n", hpn);
                copyfile(spn, hpn, NULL);
            }
        }
...
{
    _sbox_walk(opt_root, NULL, _sbox_chk_write_policy);
    /*
    _sbox_dump_sboxfs();
...
int __sbox_check_subdir_path(char *path)
{
...
void __sbox_set_write_file(char *to)
{
    struct entry ent;
    ent.path = strdup(to);
    ent.length = strlen(ent.path);
    write_dir_list.list[write_dir_list.length++] = ent;
}

void __sbox_set_clean_file(char* to)
{
    char spn[PATH_MAX];

    // Get sandbox path name (spn) from hpn.
    get_spn_from_hpn(to, spn, PATH_MAX);

    sbox_sync_parent_dirs(to, spn);

    if(__sbox_check_subdir_path(to)) {
        // If clean indicates directory, set it as directory...
        // printf("Insert into clean list %s\n", to);
        struct entry ent;
        ent.path = strdup(to);
        ent.length = strlen(ent.path);
        clean_dir_list.list[clean_dir_list.length++] = ent;
        return;
    }
    // create file
    if(create_empty_file(spn) >= 0) {
        byte md5[MD5_DIGEST_LENGTH];
        if (get_md5_from_file(spn, md5)) {
            add_md5_to_map(&os_md5map, to, md5);
        }
    }
}

void __sbox_set_copy_subdir(char *path)
{
    // If clean indicates directory, set it as directory...
    //printf("Insert into copy list %s\n", path);
    struct entry ent;
    ent.path = strdup(path);
    ent.length = strlen(ent.path);
    copy_dir_list.list[copy_dir_list.length++] = ent;
}

void __sbox_copy_file(char *from, char* to)
{
    char spn[PATH_MAX];
    ...
    struct stat sb;
    if (stat(from, &sb) == 0 && S_ISDIR(sb.st_mode)) {
    } else {
        byte md5[MD5_DIGEST_LENGTH];
        if (copyfile(from, spn, md5)) {
            add_md5_to_map(&os_md5map, from, md5);
        } else {
            //printf("Copy Failed\n");
        }
    }
...
void sbox_load_policy(char *policy)
{
...
#define COPY_NONE       (0)
#define COPY_READ_FIRST (1)
    int copy_status = COPY_NONE;
...
#define SEC_WRITE   (3)
    ...
    while (getline(&line, &len, fp) != -1) {
        ...
        // set current section
        if (line[0] == '[') {
            if (strbeg(line, "[clean]")) {
                section = SEC_CLEAN;
            } else if (strbeg(line, "[copy]")) {
                section = SEC_COPY;
            } else if (strbeg(line, "[write]")) {
                section = SEC_WRITE;
            } else {
        ...
        case SEC_CLEAN:
        {
            first_path = strdup(line);
            strip_last_newline(first_path);
            __sbox_set_clean_file(first_path);
            first_path = NULL;
            break;
        }
        case SEC_COPY:
        {
            if(copy_status == COPY_NONE) {
                copy_status = COPY_READ_FIRST;
                first_path = strdup(line);
                strip_last_newline(first_path);
                if(__sbox_check_subdir_path(first_path)) {
                    __sbox_set_copy_subdir(first_path);
                    first_path = second_path = NULL;
                    copy_status = COPY_NONE;
                }

            } else {
                second_path = strdup(line);
                strip_last_newline(second_path);
                // copy file into sbox
                __sbox_copy_file(first_path, second_path);
                // re-initialize all values
                if(first_path) free(first_path);
                if(second_path) free(second_path);
                copy_status = COPY_NONE;
                first_path = second_path = NULL;
            }
            break;
        ...
        case SEC_WRITE:
        {
            first_path = strdup(line);
            strip_last_newline(first_path);
            __sbox_set_write_file(first_path);
            first_path = NULL;
            break;
        }
        default:
